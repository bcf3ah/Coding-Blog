...
function userJWT(user){
  ...
  return jwt.encode({sub: user.id, iat: timestamp}, process.env.SECRET);//first arg is info we want to encode, 2nd arg is how we want to encrypt it (ie with our secret key). {sub: user.id, iat: timestamp} is a convention for issuing web tokens. Sub is subject sub field of the token, iat is issued at time
}
